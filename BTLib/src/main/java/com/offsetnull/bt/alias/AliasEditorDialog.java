...
public class AliasEditorDialog extends Dialog {
    ...
    public AliasEditorDialog(
            Context context,
            AliasEditorDialogDoneListener useme,
            ...
            b.setOnClickListener(
                    new View.OnClickListener() {
                        ...
                        public void onClick(View arg0) {
                            EditText pre =
                                    (EditText)
                                            AliasEditorDialog.this.findViewById(R.id.new_alias_pre);
                            EditText post =
                                    (EditText)
                                            AliasEditorDialog.this.findViewById(
                                                    R.id.new_alias_post);
                            ...
                            if (!validatePhaseTwo(
                                    pre.getText().toString(), post.getText().toString(), checker)) {
                                return;
                            }

                            if (pre != null && post != null) {
                                CheckBox carrot =
                                        (CheckBox) AliasEditorDialog.this.findViewById(R.id.carrot);
                                CheckBox dollar =
                                        (CheckBox) AliasEditorDialog.this.findViewById(R.id.dollar);
                                String prefix = "^";
                                String suffix = "$";
                                if (!carrot.isChecked()) {
                                    prefix = "";
                                ...
                                boolean checked =
                                        ((CheckBox)
                                                        AliasEditorDialog.this.findViewById(
                                                                R.id.enabledcheck))
                                                .isChecked();
                                reportto.newAliasDialogDone(
                                        prefix + pre.getText().toString() + suffix,
    ...
    private void initMatches() {
        ...
        pre.setOnEditorActionListener(
                new TextView.OnEditorActionListener() {

                    public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
                        // Log.e("EDITOR","EDITOR ACTION: " + actionId);
                        // if(event != null) Log.e("EDITOR","EVENT IS NOT NULL:" +
    ...
    private void createeditor() {
        ...
        titlebar.setText(R.string.title_modify_alias);

        Button b = (Button) findViewById(R.id.new_alias_done_button);
        b.setText(R.string.btn_done);
        ...
        b.setOnClickListener(
                new View.OnClickListener() {
                    ...
                    public void onClick(View arg0) {
                        EditText pre =
                                (EditText) AliasEditorDialog.this.findViewById(R.id.new_alias_pre);
                        EditText post =
                                (EditText) AliasEditorDialog.this.findViewById(R.id.new_alias_post);
    ...
    private boolean validatePhaseTwo(String pre, String post, Validator checker) {
        ...
        if (original_pre.startsWith("^"))
            original_pre = original_pre.substring(1, original_pre.length());
        if (original_pre.endsWith("$"))
            original_pre = original_pre.substring(0, original_pre.length() - 1);
        String invalid_name = "";
        ...
        if (offenders != null && offenders.length > 0) {
            ...
            checker.showMessage(
                    AliasEditorDialog.this.getContext(),
                    "Circular reference with aliases: " + offendersStr);
            return false;
        }
    ...
    public Object[] validateList() {
        ...
        while (tries <= maxTries && !done) {
            ...
            while (reMatch.find()) {
                ...
                if (tries > 0) {
                    if (!offenders.contains(reMatch.group(0))) {
                        offenders.add(reMatch.group(0));
                    }
                }
