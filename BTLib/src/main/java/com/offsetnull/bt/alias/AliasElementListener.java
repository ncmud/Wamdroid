...
public class AliasElementListener implements StartElementListener {
    ...
    public void start(Attributes a) {
        ...
        current_alias.setEnabled(
                (a.getValue("", "enabled") == null)
                        ? true
                        : (a.getValue("", "enabled").equals("true")) ? true : false);

        String alias_key = current_alias.getPre();
        if (current_alias.getPre().startsWith("^"))
            alias_key = alias_key.substring(1, alias_key.length());
        if (current_alias.getPre().endsWith("$"))
