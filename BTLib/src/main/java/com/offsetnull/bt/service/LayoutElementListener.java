...
/**
 * The StartElementListener responsible for keeping track of the serialized layout settings in the
 * xml file.
 */
public class LayoutElementListener implements StartElementListener {
    ...
    public final void start(final Attributes a) {
        ...
        try {
            DIMENSION_SPEC height =
                    (a.getValue("", "height") == null)
                            ? DIMENSION_SPEC.valueOf("match_parent")
                            : DIMENSION_SPEC.valueOf(a.getValue("", "height"));
            switch (height) {
                case fill_parent:
                case match_parent:
        ...
        try {
            DIMENSION_SPEC width =
                    (a.getValue("", "width") == null)
                            ? DIMENSION_SPEC.valueOf("match_parent")
                            : DIMENSION_SPEC.valueOf(a.getValue("", "width"));
            switch (width) {
                case fill_parent:
                case match_parent:
        ...
        for (int i = 0; i < numattributes; i++) {
            ...
            try {
                ...
                // orientation, height and width will be run through this. they are not RULES, so
