...
public class KeyboardCommand extends SpecialCommand {
    ...
    public Object execute(Object o, Connection c) {
        ...
        if (failed) {
            c.sendDataToWindow(
                    getErrorMessage(
                            "Keyboard (kb) special command usage:",
                            ".kb options message\n"
                                + "Options are as follows:\n"
                                + "add,popup,flush,close,clear\n"
                                + "add and popup are optional flags that will append text or popup"
                                + " the window when supplied.\n"
                                + "flush sends the current text in the input window to the"
                                + " server.\n"
                                + "close will close the keyboard if it is open.\n"
                                + "clear will erase any text that is currently in the input"
                                + " window.\n"
                                + "Example:\n"
                                + "\".kb popup reply \" will put \"reply \" into the input bar and"
                                + " pop up the keyboard.\n"
                                + "\".kb add foo\" will append foo to the current text in the input"
                                + " box and not pop up the keyboard.\n"
        ...
        Pattern p =
                Pattern.compile(
                        "^\\s*(add|popup|flush|close|clear){0,1}\\s*(add\\s+|popup\\s+|flush\\s+){0,1}(.*)$");
