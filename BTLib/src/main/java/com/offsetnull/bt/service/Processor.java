...
public class Processor {
    ...
    private HashMap<String, ArrayList<GMCPWatcher>> mGMCPTriggers =
            new HashMap<String, ArrayList<GMCPWatcher>>();
                    ...
                    if ((data[i + 1] >= TC.WILL && data[i + 1] <= TC.DONT)
                        ...
                        if (data[i + 1] == TC.SB) {
                            ...
                            if (compress) {
                                ...
                                b.rewind();
                                mReportTo.sendMessageAtFrontOfQueue(
                                        mReportTo.obtainMessage(
                                                Connection.MESSAGE_STARTCOMPRESS, b.array()));
                                if (mDebugTelnet) {
                                    String message =
                                            "\n"
                                                    + Colorizer.getTeloptStartColor()
                                                    + "IN:[IAC SB COMPRESS2 IAC SE] -BEGIN"
                                                    + " COMPRESSION-"
    ...
    public final void dispatchIAC(final byte action, final byte option) {
        ...
        if (mDebugTelnet) {
            message =
                    Colorizer.getTeloptStartColor()
                            + "IN:["
                            + TC.decodeIAC(new byte[] {TC.IAC, action, option})
                            + "]"
                            + " ";
            message +=
                    Colorizer.getTeloptStartColor()
            ...
            if (mDebugTelnet) {
                String message =
                        "\n"
                                + Colorizer.getTeloptStartColor()
                                + "IN:["
                                + TC.decodeSUB(negotiation)
                                + "]"
                                + Colorizer.getResetColor()
                                + "\n";
                mReportTo.sendMessageDelayed(
                        mReportTo.obtainMessage(Connection.MESSAGE_PROCESSORWARNING, message), 1);
            }
            ...
            try {
                ...
                if (list != null) {
                    for (int i = 0; i < list.size(); i++) {
                        ...
                        Message gmsg =
                                mReportTo.obtainMessage(Connection.MESSAGE_GMCPTRIGGERED, tmpdata);
