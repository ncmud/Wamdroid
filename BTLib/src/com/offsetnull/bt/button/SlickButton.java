...
public class SlickButton extends View {
    ...
    public void setData(SlickButtonData in) {
        ...
        rect.set(
                data.getX() - (int) ((data.getWidth() * density) / 2),
                data.getY() - (int) ((data.getHeight() * density) / 2),
                data.getX() + (int) ((data.getWidth() * density) / 2),
                data.getY() + (int) ((data.getHeight() * density) / 2));
    }
    ...
    private void doButtonSetChange() {
        Message swaptoset =
                dispatcher.obtainMessage(MainWindow.MESSAGE_CHANGEBUTTONSET, data.getTargetSet());
    ...
    public void updateRect() {
        rect.set(
                data.getX() - (int) ((data.getWidth() / 2) * density),
                data.getY() - (int) ((data.getHeight() * density) / 2),
                data.getX() + (int) ((data.getWidth() * density) / 2),
                data.getY() + (int) ((data.getHeight() * density) / 2));
    }
    ...
    public boolean onTouchEvent(MotionEvent e) {
        ...
        if (fullScreenShift > 0) {
            // Log.e("BUTTON","START SHIFT TRANSLATION: " + e.getX(e.getPointerId(0)) + " " +
            // e.getY(e.getPointerId(0)));
            e.setLocation(e.getX(e.getPointerId(0)), e.getY(e.getPointerId(0)) - fullScreenShift);
            // Log.e("BUTTON","FINISH SHIFT TRANSLATION: " + e.getX(e.getPointerId(0)) + " " +
            // e.getY(e.getPointerId(0)));
        ...
        if (e.getAction() == MotionEvent.ACTION_DOWN) {
            if (!preserve_lock) {
                // Log.e("SLICKBUTTON","SETTING ORIGINAL DATA TO IDENTIFY MYSELF WHEN THE DIALOG
                // GOES.");
    ...
    public void onDraw(Canvas c) {
        ...
        if (doing_flip) {
            ...
            tsize =
                    opts.measureText(
                            (data.getFlipLabel().equals(""))
                                    ? data.getLabel()
                                    : data.getFlipLabel());
            // c.drawText((data.getFlipLabel().equals("")) ? data.getLabel() : data.getFlipLabel(),
            // data.getX()-tsize/2, data.getY(), opts);
            c.drawText(
                    (data.getFlipLabel().equals("")) ? data.getLabel() : data.getFlipLabel(),
                    data.getX() - tsize / 2,
                    data.getY() + (int) ((data.getLabelSize() * density) / 2),
                    opts);
        ...
        if (moving) {
            mMovingRect.set(
                    data.getX() - (int) ((data.getWidth() * density) / 2) + 5,
                    data.getY() - (int) ((data.getHeight() * density) / 2) + 5,
                    data.getX() + (int) ((data.getWidth() * density) / 2) - 5,
                    data.getY() + (int) ((data.getHeight() * density) / 2) - 5);
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case MSG_BEGINMOVE:
                    outer.moving = true;
                    outer.invalidate();
                    // SlickButton.this.performHapticFeedback(HapticFeedbackConstants.VIRTUAL_KEY,
                ...
                case MSG_DELETE:
                    ...
                    if (!outer.lockEdit) {
                        // Message deleme = deleter.obtainMessage(ByteView.MSG_DELETEBUTTON,
                        // SlickButton.this);
