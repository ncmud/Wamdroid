...
public class OptionNegotiator {
    ...
    public final byte[] processCommand(final byte first, final byte second, final byte third) {
        ...
        // final byte ATCP_CUSTOM = (byte)0xC8; //200 -- ATCP protocol,
        // http://www.ironrealms.com/rapture/manual/files/FeatATCP-txt.html
        // final byte AARD_CUSTOM = (byte)0x66; //102 -- Aardwolf custom,
    ...
    public final byte[] getSubnegotiationResponse(final byte[] sequence) {
        // first some asserts
        if (sequence[0] != TC.IAC
                || sequence[1] != TC.SB
                || sequence[sequence.length - 2] != TC.IAC
                || sequence[sequence.length - 1] != TC.SE) {
    ...
    /**
     * Reset method, this is just to let the processing routines know that the TTYPE stack is reset
     * to the beginning.
     */
