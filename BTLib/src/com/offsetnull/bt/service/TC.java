...
public final class TC {
    ...
    public static final byte ATCP_CUSTOM = (byte) 0xC8; // 200 -- ATCP protocol,

    // http://www.ironrealms.com/rapture/manual/files/FeatATCP-txt.html

    /** test constant for aard custom protocol. */
    public static final byte AARD_CUSTOM = 0x66; // 102 -- Aardwolf custom,
        ...
        if (datalen > 0) {
            ...
            switch (in[2]) {
                ...
                case GMCP:
                    ...
                    if (in.length
                            > GMCP_SUB_MIN_LENGTH) { // has to be if valid gmcp message IAC SB GMCP
                        // <lotsofdata> IAC SE
                ...
                case NAWS:
                    ret +=
                            Integer.toString(LSB_MASK & in[NAWS_POS_1])
                                    + " "
                                    + Integer.toString(LSB_MASK & in[NAWS_POS_2])
                                    + " "
                                    + Integer.toString(LSB_MASK & in[NAWS_POS_3])
                                    + " "
                                    + Integer.toString(LSB_MASK & in[NAWS_POS_4])
                                    + " IAC SE";
                    return ret;
