...
public class LayoutGroupElementListener implements ElementListener {
    ...
    /**
     * The current working window object, this is actually a single window object that gets re-used
     * for every inflated window.
     */
     ...
     */
    public LayoutGroupElementListener(
            final WindowToken currentWindow, final LayoutElementListener sub) {
    ...
    public final void start(final Attributes a) {
        ...
        try {
            LayoutGroup.LAYOUT_TYPE tmp =
                    LayoutGroup.LAYOUT_TYPE.valueOf(target.toLowerCase(Locale.US));
