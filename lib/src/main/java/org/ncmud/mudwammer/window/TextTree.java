...
public class TextTree {
    ...
    private static final byte BRACKET = (byte) 0x5B;
    private final byte NEWLINE = (byte) 0x0A;
    // private final byte CARRIAGE = (byte)0x0D;
    private static final byte m = (byte) 0x6D;
    private static final byte SEMI = (byte) 0x3B;

    private static final byte b0 = (byte) 0x30;
    private static final byte b1 = (byte) 0x31;
    private static final byte b2 = (byte) 0x32;
    private static final byte b3 = (byte) 0x33;
    private static final byte b4 = (byte) 0x34;
    private static final byte b5 = (byte) 0x35;
    private static final byte b6 = (byte) 0x36;
    private static final byte b7 = (byte) 0x37;
    private static final byte b8 = (byte) 0x38;
    private static final byte b9 = (byte) 0x39;
    ...
    boolean appendLast =
            false; // for marking when the addtext call has ended with a newline or not.
    ...
    public int addBytesImpl(byte[] data) throws UnsupportedEncodingException {
        ...
        // Log.e("TREE","ADDING: " + data.length + " bytes, buffer has " + totalbytes + " total
        // bytes. " + projected + " projected.");
        ...
        if (appendLast) { // yay appendLast is over. now just look at the last line of the buffer,
            // parse through it and find if the last text in it (not color) was a
            // newline.
            // if(mLines.size() > 0) {
            tmp = mLines.remove(0); // dont worry kids, it'll be appended back.
            totalbytes -=
                    tmp.bytes; // this will be added back too, this is just to avoid memory leaking
            ldata = tmp.getData();
            ...
            switch (data[i]) {
                case ESC:
                    ...
                    if (cb.position() > 0) {
                        ...
                        // Log.e("TREE","APPEND DUE TO UNTERMINATED ANSI SEQUENCE:"  +
                        // deColorLine(tmp));
                        addLine(tmp);
                        linesadded += tmp.breaks + 1;
                        // Log.e("TREE","WARNING: UNTERMINATED ASCII SEQUENCE: " + new
                        // String(holdover,encoding));
                        int endcount = this.getBrokenLineCount();
                        // prune();
        ...
        if (sb.position() > 0) {
            ...
            // Log.e("TREE",">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>NOT ENDED BY NEWLINE:" +
            // deColorLine(tmp));
    ...
    public class Line {
        ...
        public void updateData() {
            ...
            // int nonWhiteSpaceRun = 0; //tracker for how many characters have accumulated without
            // whitespace
                ...
                if (charsinline > breakAt) {
                    ...
                    if (wordWrap) {
                        if (whiteSpaceFound) {
                            ...
                            boolean found = false;
                            // i.previous(); //advance back because we are on the right hand side of
                            // the unit that broke.
                        ...
                        // if the number of non whitespace characters is < breakAt, then we should
                        // go back and search for the whitespace
        ...
        private int breakAt(ListIterator<Unit> i, Unit u, int amount, int length) {
            ...
            if (amount == 0) {
                ...
                if (i.hasNext()) {
                    i.next(); // advance the cursor so we don't go through and delete
                    // existing breaks.
                }
                breaks += 1;
                charsinline = 0;
                ...
                if (working >= startcol) {
                    ...
                    builder.append(
                            ((Text) u)
                                    .getString()
                                    .substring(
                                            ((Text) u).bytecount - (working - startcol),
                                            ((Text) u).bytecount));
                        ...
                        if (((Text) u).bytecount == 1) {
                            ...
                            builder.append(
                                    ((Text) u)
                                            ...
                                            .substring(
                                                    0,
                                                    endcol - (working - ((Text) u).bytecount - 1)));
                        }
                        done = true;
                    } else {
                        builder.append(((Text) u).getString());
