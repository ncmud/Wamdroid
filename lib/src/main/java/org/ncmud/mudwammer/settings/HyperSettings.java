...
public class HyperSettings {
    ...
    public static final int DEFAULT_HYPERLINK_COLOR = 0xFF3333AA;
    ...
    private HashMap<String, Vector<SlickButtonData>> ButtonSets =
    ...
    public static String writeXml2(HyperSettings data) {
        ...
        try {
            ...
            out.attribute(
                    "", BaseParser.ATTR_LINESIZE, Integer.valueOf(data.getLineSize()).toString());
            out.attribute(
                    "",
                    BaseParser.ATTR_SPACEEXTRA,
                    Integer.valueOf(data.getLineSpaceExtra()).toString());
            out.attribute(
                    "", BaseParser.ATTR_MAXLINES, Integer.valueOf(data.getMaxLines()).toString());
            out.attribute("", BaseParser.ATTR_FONTNAME, data.getFontName());
            out.attribute("", BaseParser.ATTR_FONTPATH, data.getFontPath());
            out.attribute(
                    "", BaseParser.ATTR_USEEXTRACTUI, (data.isUseExtractUI()) ? "true" : "false");
            out.attribute(
                    "",
                    BaseParser.ATTR_SUGGESTIONS,
                    (data.isAttemptSuggestions()) ? "true" : "false");
            out.attribute("", BaseParser.ATTR_KEEPLAST, (data.isKeepLast()) ? "true" : "false");
            out.attribute(
                    "",
                    BaseParser.ATTR_BACKSPACEFIX,
                    (data.isBackspaceBugFix()) ? "true" : "false");
            out.attribute(
                    "",
                    BaseParser.ATTR_AUTOLAUNCHEDITOR,
                    (data.isAutoLaunchButtonEdtior()) ? "true" : "false");
            out.attribute(
                    "", BaseParser.ATTR_DISABLECOLOR, (data.isDisableColor()) ? "true" : "false");
            out.attribute(
                    "",
                    BaseParser.ATTR_OVERRIDEHF,
                    (data.getHapticFeedbackMode().equals(""))
                            ? "auto"
                            : data.getHapticFeedbackMode());
            out.attribute(
                    "",
                    BaseParser.ATTR_HFONPRESS,
                    (data.getHapticFeedbackOnPress().equals(""))
                            ? "auto"
                            : data.getHapticFeedbackOnPress());
            out.attribute(
                    "",
                    BaseParser.ATTR_HFONFLIP,
                    (data.getHapticFeedbackOnFlip().equals(""))
                            ? "auto"
                            : data.getHapticFeedbackOnFlip());
            out.attribute("", BaseParser.ATTR_ENCODING, data.getEncoding());
            out.attribute(
                    "", BaseParser.ATTR_KEEPSCREENON, (data.isKeepScreenOn()) ? "true" : "false");
            out.attribute("", BaseParser.ATTR_FULLSCREEN, (data.isFullScreen()) ? "true" : "false");
            out.attribute(
                    "", BaseParser.ATTR_ROUNDBUTTONS, (data.isRoundButtons()) ? "true" : "false");
            // out.attribute("", BaseParser.ATTR_FITBUTTONS, (data.isShowFitMessage()) ? "true" :
            // "false");
            ...
            out.attribute(
                    "",
                    BaseParser.ATTR_REMOVEEXTRACOLOR,
                    (data.isRemoveExtraColor()) ? "true" : "false");
            out.attribute(
                    "", BaseParser.ATTR_DEBUGTELNET, (data.isDebugTelnet()) ? "true" : "false");
            ...
            out.attribute(
                    "",
                    BaseParser.ATTR_HYPERLINKCOLOR,
                    Integer.toHexString(data.getHyperLinkColor()));

            out.attribute(
                    "",
                    BaseParser.ATTR_HYPERLINKENABLED,
                    (data.isHyperLinkEnabled()) ? "true" : "false");
            ...
            out.attribute(
                    "", BaseParser.ATTR_SEMINEWLINE, (data.isSemiIsNewLine()) ? "true" : "false");
            // out.attribute("", BaseParser.ATTR_USEEXTRACTUI, (data.isUseExtractUI()) ? "true" :
            // "false");
            out.attribute(
                    "",
                    BaseParser.ATTR_THROTTLEBACKGROUND,
                    (data.isThrottleBackground()) ? "true" : "false");
            out.attribute(
                    "", BaseParser.ATTR_PROCESSPERIOD, data.isProcessPeriod() ? "true" : "false");
            if (!data.isEchoAliasUpdates())
                out.attribute("", BaseParser.ATTR_ECHOALIASUPDATE, "false");
            out.attribute(
                    "", BaseParser.ATTR_WIFIKEEPALIVE, data.isKeepWifiActive() ? "true" : "false");
            out.attribute("", BaseParser.ATTR_LOCALECHO, (data.isLocalEcho()) ? "true" : "false");
            out.attribute(
                    "", BaseParser.ATTR_BELLVIBRATE, (data.isVibrateOnBell()) ? "true" : "false");
            out.attribute(
                    "", BaseParser.ATTR_BELLNOTIFY, (data.isNotifyOnBell()) ? "true" : "false");
            out.attribute(
                    "", BaseParser.ATTR_BELLDISPLAY, (data.isDisplayOnBell()) ? "true" : "false");
            ...
            for (String key : buttonsets) {
                ...
                if (setdefaults.getPrimaryColor() != SlickButtonData.DEFAULT_COLOR)
                    out.attribute(
                            "",
                            BaseParser.ATTR_PRIMARYCOLOR,
                            Integer.toHexString(setdefaults.getPrimaryColor()));
                if (setdefaults.getSelectedColor() != SlickButtonData.DEFAULT_SELECTED_COLOR)
                    out.attribute(
                            "",
                            BaseParser.ATTR_SELECTEDCOLOR,
                            Integer.toHexString(setdefaults.getSelectedColor()));
                if (setdefaults.getFlipColor() != SlickButtonData.DEFAULT_FLIP_COLOR)
                    out.attribute(
                            "",
                            BaseParser.ATTR_FLIPCOLOR,
                            Integer.toHexString(setdefaults.getFlipColor()));
                if (setdefaults.getLabelColor() != SlickButtonData.DEFAULT_LABEL_COLOR)
                    out.attribute(
                            "",
                            BaseParser.ATTR_LABELCOLOR,
                            Integer.toHexString(setdefaults.getLabelColor()));
                if (setdefaults.getButtonHeight() != SlickButtonData.DEFAULT_BUTTON_HEIGHT)
                    out.attribute(
                            "",
                            BaseParser.ATTR_BUTTONHEIGHT,
                            Integer.valueOf(setdefaults.getButtonHeight()).toString());
                if (setdefaults.getButtonWidth() != SlickButtonData.DEFAULT_BUTTON_WDITH)
                    out.attribute(
                            "",
                            BaseParser.ATTR_BUTTONWIDTH,
                            Integer.valueOf(setdefaults.getButtonWidth()).toString());
                if (setdefaults.getLabelSize() != SlickButtonData.DEFAULT_LABEL_SIZE)
                    out.attribute(
                            "",
                            BaseParser.ATTR_LABELSIZE,
                            Integer.valueOf(setdefaults.getLabelSize()).toString());
                if (setdefaults.getFlipLabelColor() != SlickButtonData.DEFAULT_FLIPLABEL_COLOR)
                    out.attribute(
                            "",
                            BaseParser.ATTR_FLIPLABELCOLOR,
                            Integer.toHexString(setdefaults.getFlipLabelColor()));
                if (setdefaults.isLocked() != ColorSetSettings.DEFAULT_LOCKED)
                    out.attribute(
                            "", BaseParser.ATTR_LOCKED, setdefaults.isLocked() ? "true" : "false");
                if (setdefaults.isLockNewButtons() != ColorSetSettings.DEFAULT_LOCKNEWBUTTONS)
                    out.attribute(
                            "",
                            BaseParser.ATTR_LOCKNEWBUTTONS,
                            setdefaults.isLockNewButtons() ? "true" : "false");
                if (setdefaults.isLockMoveButtons() != ColorSetSettings.DEFAULT_LOCKMOVEBUTTONS)
                    out.attribute(
                            "",
                            BaseParser.ATTR_LOCKMOVEBUTTONS,
                            setdefaults.isLockMoveButtons() ? "true" : "false");
                if (setdefaults.isLockEditButtons() != ColorSetSettings.DEFAULT_LOCKEDITBUTTONS)
                    out.attribute(
                            "",
                            BaseParser.ATTR_LOCKEDITBUTTONS,
                            setdefaults.isLockEditButtons() ? "true" : "false");
                ...
                for (SlickButtonData button : the_set) {
                    ...
                    out.attribute(
                            "", BaseParser.ATTR_XPOS, Integer.valueOf(button.getX()).toString());
                    out.attribute(
                            "", BaseParser.ATTR_YPOS, Integer.valueOf(button.getY()).toString());
                    ...
                    if (!button.getFlipCommand().equals(""))
                    ...
                    if (button.getWidth() != setdefaults.getButtonWidth())
                        out.attribute(
                                "",
                                BaseParser.ATTR_WIDTH,
                                Integer.valueOf(button.getWidth()).toString());
                    if (button.getHeight() != setdefaults.getButtonHeight())
                        out.attribute(
                                "",
                                BaseParser.ATTR_HEIGHT,
                                Integer.valueOf(button.getHeight()).toString());
                    if (button.getPrimaryColor() != setdefaults.getPrimaryColor())
                        out.attribute(
                                "",
                                BaseParser.ATTR_PRIMARYCOLOR,
                                Integer.toHexString(button.getPrimaryColor()));
                    if (button.getSelectedColor() != setdefaults.getSelectedColor())
                        out.attribute(
                                "",
                                BaseParser.ATTR_SELECTEDCOLOR,
                                Integer.toHexString(button.getSelectedColor()));
                    if (button.getFlipColor() != setdefaults.getFlipColor())
                        out.attribute(
                                "",
                                BaseParser.ATTR_FLIPCOLOR,
                                Integer.toHexString(button.getFlipColor()).toString());
                    if (button.getLabelColor() != setdefaults.getLabelColor())
                        out.attribute(
                                "",
                                BaseParser.ATTR_LABELCOLOR,
                                Integer.toHexString(button.getLabelColor()));
                    if (button.getLabelSize() != setdefaults.getLabelSize())
                        out.attribute(
                                "",
                                BaseParser.ATTR_LABELSIZE,
                                Integer.valueOf(button.getLabelSize()).toString());
                    if (button.getFlipLabelColor() != setdefaults.getFlipLabelColor())
                        out.attribute(
                                "",
                                BaseParser.ATTR_FLIPLABELCOLOR,
                                Integer.toHexString(button.getFlipLabelColor()));
                    if (!button.getFlipLabel().equals(""))
                        out.attribute("", BaseParser.ATTR_FLIPLABEL, button.getFlipLabel());
            ...
            for (TriggerData trigger : data.getTriggers().values()) {
                if (trigger.isSave()) {
                    ...
                    out.attribute(
                            "",
                            BaseParser.ATTR_TRIGGERLITERAL,
                            trigger.isInterpretAsRegex() ? "true" : "false");
                    out.attribute(
                            "",
                            BaseParser.ATTR_TRIGGERONCE,
                            trigger.isFireOnce() ? "true" : "false");
                    if (trigger.isHidden())
                        out.attribute("", BaseParser.ATTR_TRIGGERHIDDEN, "true");
                    out.attribute(
                            "",
                            BaseParser.ATTR_TRIGGERENEABLED,
    ...
    private static void OutputResponders(XmlSerializer out, List<TriggerResponder> responders)
            ...
            switch (responder.getType()) {
                case NOTIFICATION:
                    ...
                    if (notify.isUseDefaultLight()) {
                        ...
                        out.attribute(
                                "",
                                BaseParser.ATTR_LIGHTCOLOR,
                                Integer.toHexString(notify.getColorToUse()));
                    } else {
                    ...
                    if (notify.isUseDefaultVibrate()) {
                        ...
                        out.attribute(
                                "",
                                BaseParser.ATTR_VIBRATELENGTH,
                                Integer.toString(notify.getVibrateLength()));
                    } else {
                    ...
                    out.attribute(
                            "",
                            BaseParser.ATTR_NEWNOTIFICATION,
                            (notify.isSpawnNewNotification()) ? "true" : "false");
                    out.attribute(
                            "",
                            BaseParser.ATTR_USEONGOING,
                            (notify.isUseOnGoingNotification()) ? "true" : "false");
                ...
                case TOAST:
                    ...
                    out.attribute(
                            "",
                            BaseParser.ATTR_TOASTDELAY,
                            Integer.valueOf(toasty.getDelay()).toString());
