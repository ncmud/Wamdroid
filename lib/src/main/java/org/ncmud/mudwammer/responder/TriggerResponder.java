...
public abstract class TriggerResponder {
    ...
    public void removeFireType(FIRE_WHEN in) {
        ...
        // Log.e("RESPONDER","REMOVED " + in.getString() + " FIRE TYPE NOW " +
        // fireType.getString());
    }

    public abstract boolean doResponse(
            Context c,
            TextTree tree,
            int lineNumber,
    ...
    public String translate(String input, HashMap<String, String> map) {
        ...
        while (replacer.find()) {
            ...
            replacer.appendReplacement(
                    output,
                    replacetext); // append with map data if exists, use the group count if not.
        }
    ...
    public abstract void saveResponderToXML(XmlSerializer out)
