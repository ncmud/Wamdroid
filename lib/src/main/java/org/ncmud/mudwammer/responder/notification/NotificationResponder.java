...
public class NotificationResponder extends TriggerResponder {
    ...
    public boolean doResponse(
        ...
        int resId =
                c.getResources()
                        .getIdentifier(
                                ConfigurationLoader.getConfigurationValue("notificationIcon", c),
                                "drawable",
                                c.getPackageName());
        ...
        // Intent(c,org.ncmud.mudwammer.window.MainWindow.class);
        ...
        try {
            w = Class.forName("org.ncmud.mudwammer.window.MainWindow", false, cl);
        ...
        try {
            notificationIntent.setClass(
                    c.createPackageContext(c.getPackageName(), Context.CONTEXT_INCLUDE_CODE), w);
        } catch (NameNotFoundException e) {
            e.printStackTrace();
        }

        /*} else {
        	notificationIntent = new Intent("org.ncmud.mudwammer.window.MainWindow"+".TEST_MODE");
        	String apkName = null;
        	try {
        		apkName = c.getPackageManager().getApplicationInfo("org.ncmud.mudwammer.test", 0).sourceDir;
        	} catch (NameNotFoundException e1) {
        		// TODO Auto-generated catch block
        		e1.printStackTrace();
        	}
        	...
        	try {
        		notificationIntent.setClass(c.createPackageContext("org.ncmud.mudwammer.test", Context.CONTEXT_INCLUDE_CODE), w);
        	} catch (NameNotFoundException e) {
        		// TODO Auto-generated catch block
        		e.printStackTrace();
        	}
        ...
        PendingIntent contentIntent =
                PendingIntent.getActivity(
                        c, myTriggerId, notificationIntent, PendingIntent.FLAG_UPDATE_CURRENT);

        // note.setLatestEventInfo(c, title, message, contentIntent);
        String channelId =
                ConfigurationLoader.getConfigurationValue("ongoingNotificationLabel", c)
                        + "_service"
                        + "_"
                        + displayname;
        ...
        builder.setOnlyAlertOnce(true)
                ...
                .setSmallIcon(resId);
        // note.flags = Notification.FLAG_ONLY_ALERT_ONCE | Notification.FLAG_SHOW_LIGHTS |
        ...
        // note.vibrate = vp;
        NM.cancel(myTriggerId); // cancel my id if i am a not spawn new kind of guy, if that is true
        // this shouldn't be a problem here, as the id doens't exist in the
