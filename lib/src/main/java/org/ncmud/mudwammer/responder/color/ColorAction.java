...
import org.ncmud.mudwammer.window.TextTree.Color;
import org.ncmud.mudwammer.window.TextTree.Line;
import org.ncmud.mudwammer.window.TextTree.Text;
...
public class ColorAction extends TriggerResponder {
    ...
    public boolean doResponse(
        ...
        if (preEmptiveChop) {
            ...
            Text post =
                    line.newText(
                            ((Text) u).getString().substring(length - preEmptiveChopAt, length));
            ...
            if (chopAt > 0) {
                ...
                Text post =
                        line.newText(((Text) chop).getString().substring(length - chopAt, length));
                // insert bleed color
                newLine.add(bleed);
