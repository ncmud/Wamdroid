...
public class SpeedWalkDirectionEditorDialog extends Dialog {
    ...
    public SpeedWalkDirectionEditorDialog(
            Context context, DirectionEditorDoneListener doneListener, StellarService service) {
        super(context);
    ...
    public void onCreate(Bundle b) {
        ...
        if (isEditor) {
            ...
            findViewById(R.id.new_sw_done_button)
                    .setOnClickListener(
        ...
        findViewById(R.id.new_sw_cancel_button)
                .setOnClickListener(
                        new View.OnClickListener() {
    ...
    private boolean validateEntries() {
        ...
        if (direction.getText().toString().length() > 1) {
            checker.showMessage(
                    SpeedWalkDirectionEditorDialog.this.getContext(),
                    "Direction field can not be more than 1 character.");
            return false;
        }
        ...
        try {
            ...
            checker.showMessage(
                    SpeedWalkDirectionEditorDialog.this.getContext(),
                    "Direction field can not be a number. " + val + " is invalid.");
            return false;
        ...
        HashMap<String, DirectionData> tmp =
                (HashMap<String, DirectionData>) service.getDirectionData();
        for (DirectionData d : tmp.values()) {
            if (isEditor) {
                if (d.getDirection().equals(direction.getText().toString())
                        && !d.getDirection().equals(oldData.getDirection())) {
                    checker.showMessage(
                            SpeedWalkDirectionEditorDialog.this.getContext(),
                            d.getDirection() + " is already used as a direction.");
                    return false;
                }
            } else {
                if (d.getDirection().equals(direction.getText().toString())) {
        ...
        if (direction.getText().toString().equals(";")
                || direction.getText().toString().equals(",")) {
            checker.showMessage(
                    SpeedWalkDirectionEditorDialog.this.getContext(),
                    "\"" + direction.getText().toString() + "\" can not be used as a direction.");
            return false;
        }
