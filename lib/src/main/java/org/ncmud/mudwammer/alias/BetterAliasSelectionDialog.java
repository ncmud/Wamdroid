...
import org.ncmud.mudwammer.window.BaseSelectionDialog;
...
public class BetterAliasSelectionDialog extends PluginFilterSelectionDialog
        implements BaseSelectionDialog.UtilityToolbarListener, AliasEditorDialogDoneListener {

    HashMap<String, AliasData> dataMap;
    String[] sortedKeys;

    public BetterAliasSelectionDialog(Context context, StellarService service) {
    ...
    public void onButtonPressed(View v, int row, int index) {
        ...
        AliasEditorDialog editor =
                new AliasEditorDialog(
                        BetterAliasSelectionDialog.this.getContext(),
                        BetterAliasSelectionDialog.this,
                        d.getPre(),
                        d.getPost(),
                        row,
                        d,
                        service,
                        computeNames(d.getPre()),
                        currentPlugin);
        editor.show();
    }
    ...
    public void onHelp() {
        Intent web_help =
                new Intent(
                        Intent.ACTION_VIEW,
                        Uri.parse("http://bt.happygoatstudios.com/?view=aliases"));
    ...
    private void buildList() {
        ...
        // pull the list down, clear out the items list, populate it, and call the superclass to
        // reload the table.
    ...
    public void willShowToolbar(LinearLayout toolbar, int row) {
        // this will be called before the toolbar is shown, it will give the implementer the option
        // to set up on/off lock/unlock etc.
    ...
    public void newAliasDialogDone(String pre, String post, boolean enabled) {
        ...
        aliasEditorDoneHandler.sendMessageDelayed(
                aliasEditorDoneHandler.obtainMessage(100, newAlias), 10);
    }
