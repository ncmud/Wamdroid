...
import org.ncmud.mudwammer.button.ButtonEditorDialog.COLOR_FIELDS;
...
public class ButtonSetEditor extends Dialog implements ColorPickerDialog.OnColorChangedListener {
    ...
    public ButtonSetEditor(
            Context context,
            StellarService the_service,
            String selected_set,
            Handler use_this_handler) {
        super(context);
        service = the_service;
        set = selected_set;
    ...
    public void onCreate(Bundle b) {
        ...
        normalColor.setOnClickListener(
                new View.OnClickListener() {

                    public void onClick(View arg0) {
                        ColorPickerDialog diag =
                                new ColorPickerDialog(
                                        ButtonSetEditor.this.getContext(),
                                        ButtonSetEditor.this,
                                        newsettings.getPrimaryColor());
                        diag.show();
                    }
                });
                    ...
                    public void onClick(View arg0) {
                        ColorPickerDialog diag =
                                new ColorPickerDialog(
                                        ButtonSetEditor.this.getContext(),
                                        ButtonSetEditor.this,
                                        newsettings.getLabelColor());
                        diag.show();
                    }
                });

        flipColor.setOnClickListener(
                new View.OnClickListener() {
                    ...
                    public void onClick(View arg0) {
                        ColorPickerDialog diag =
                                new ColorPickerDialog(
                                        ButtonSetEditor.this.getContext(),
                                        ButtonSetEditor.this,
                                        newsettings.getFlipColor());
                        diag.show();
                    }
                });

        flipLabelColor.setOnClickListener(
                new View.OnClickListener() {
                    ...
                    public void onClick(View arg0) {
                        ...
                        for (String str : takenNames) {
                            if (nameEditor.getText().toString().equals(str)
                                    && !nameEditor.getText().toString().equals(set)) {
                                checker.showMessage(
                                        ButtonSetEditor.this.getContext(),
                                        nameEditor.getText().toString()
                                                + " is an existing button set.");
                                return;
                            }
                        }

                        newsettings.setButtonHeight(
                                Integer.parseInt(buttonHeight.getText().toString()));
                        newsettings.setButtonWidth(
                                Integer.parseInt(buttonWidth.getText().toString()));
                        ...
                        if (!(nameEditor.getText().toString().equals(set))) {
                            //
                            // service.updateAndRenameSet(set, nameEditor.getText().toString(),
                            // newsettings);
                            //
                            notifychanged.sendMessage(
                                    notifychanged.obtainMessage(
                                            101, nameEditor.getText().toString()));
                            ButtonSetEditor.this.dismiss();
                            return;
                        }
                        ...
                        if (newsettings.equals(oldsettings)) {
                            ...
                            // changes made, notify the service that the change has been made, and
                            // notify the settings dialog that when it exits it needs to reload
                            // whatever button set changed.
                            //
