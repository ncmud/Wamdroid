...
import org.ncmud.mudwammer.settings.BaseParser;
...
public class LauncherSAXParser extends BaseParser {
    ...
    public LauncherSettings load() {
        ...
        launcher.setStartElementListener(
                new StartElementListener() {

                    public void start(Attributes a) {
                        tmp.setCurrentVersion(
                                (a.getValue("", BaseParser.ATTR_VERSION) == null)
                                        ? "v1.0.4"
                                        : a.getValue("", BaseParser.ATTR_VERSION));
                    }
                });
        ...
        item.setStartElementListener(
                new StartElementListener() {
                    ...
                    public void start(Attributes a) {
                        current_item.setDisplayName(
                                (a.getValue("", BaseParser.ATTR_NAME) == null)
                                        ? "Mud"
                                        : a.getValue("", BaseParser.ATTR_NAME));
                        current_item.setHostName(
                                (a.getValue("", BaseParser.ATTR_HOST) == null)
                                        ? "host not set"
                                        : a.getValue("", BaseParser.ATTR_HOST));
                        current_item.setPortString(
                                (a.getValue("", BaseParser.ATTR_PORT) == null)
                                        ? "4002"
                                        : a.getValue("", BaseParser.ATTR_PORT));
                        current_item.setLastPlayed(
                                (a.getValue("", BaseParser.ATTR_DATEPLAYED) == null)
                                        ? "11-25-2010 11:53am"
        ...
        launcher.setEndElementListener(
