...
public class SpeedwalkCommand extends SpecialCommand {
    ...
    private org.ncmud.mudwammer.service.AliasManager.Data mData = null;

    public SpeedwalkCommand(
            HashMap<String, DirectionData> directions,
            org.ncmud.mudwammer.service.AliasManager.Data data) {
    ...
    public Object execute(Object o, Connection c) {
        ...
        if (str.equals("") || str.equals(" ")) {
            c.sendDataToWindow(
                    getErrorMessage(
                            "Speedwalk (run) special command usage:",
                            ".run directions\n"
                                + "Direction ordinal to command mappings are editable, press"
                                + " MENU->More->Speedwalk Configuration for more info. The default"
                                ...
                                + " k: southwest\n"
                                + " l: southeast\n"
                                + "directions may be prefeced with an integer value to run that"
                                + " many times.\n"
                                + "Commands may be inserted into the direction stream with"
                                + " commas,\n"
                                + "directions may be resumed by entering another comma followed by"
                                + " directions.\n"
                                + "Example:\n"
                                + "\".run 3desw2n\", will send d;d;d;e;s;w;n;n to the server.\n"
                                + "\".run jlk3n3j\", will send se;nw;sw;n;n;n;se;se;se to the"
                                + " server.\n"
                                + "\".run 3ds,open door,3w\" will send d;d;d;s;open door;w;w;w to"
                                + " the server.\n"));
            return null;
        }
        ...
        // mData = new org.ncmud.mudwammer.service.AliasManager.Data();
