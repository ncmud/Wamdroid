...
/**
 * The implementation of the GMCP Table. It keeps track of the data using hashmaps to store tables
 * at string paths.
 */
public class GMCPData {
    ...
    public final void putData(
            ...
            final HashMap<String, Object> node,
        ...
        if (node.containsKey(key)) {
            // grab it and insert the rest of the data, if there is no rest, dump the stuff into the
            // current map.
            if (rest.equals("")) {
                Object o = node.get(key);
                if (o == null) {
                    Log.e("GMCP", "WARNING! KEY: " + key + " is null!");
                } // holy god. never should this happen.
                if (!(o instanceof HashMap<?, ?>)) {
                    Log.e("GMCP", "WARNING! KEY: " + key + " is not a hashmap!");
                }
     ...
     *
     * @param path The path to fetch, e.g. char.vitals.hp.
     * @param node The gmcp data table to fetch, the recursive function starts with the top level
     *     table.
     * @return The gmcp data for the node.
     */
    ...
    private void insertData(
        ...
        while (keys.hasNext()) {
            ...
            try {
                ...
                // this is an ugly situation here where the object.getJSONObject call above must be
                // done
     ...
     */
    @SuppressWarnings("unchecked")
    private HashMap<String, Object> findNextTable(
            final String key,
            final String[] parts,
            final int pIndex,
            final HashMap<String, Object> node) {
        ...
        if (node.containsKey(key)) {
            if (index == parts.length - 1) {
                ...
                return findNextTable(
                        parts[index], parts, index, (HashMap<String, Object>) node.get(key));
            }
        } else {
            return null;
        }
