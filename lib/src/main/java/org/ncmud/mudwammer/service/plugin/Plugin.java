package org.ncmud.mudwammer.service.plugin;
...
import org.ncmud.mudwammer.responder.IteratorModifiedException;
...
import org.ncmud.mudwammer.script.ScriptData;
import org.ncmud.mudwammer.service.Colorizer;
import org.ncmud.mudwammer.service.Connection;
import org.ncmud.mudwammer.service.ConnectionPluginCallback;
...
import org.ncmud.mudwammer.service.plugin.function.NoteFunction;
import org.ncmud.mudwammer.service.plugin.function.TriggerEnabledFunction;
import org.ncmud.mudwammer.service.plugin.settings.BaseOption;
import org.ncmud.mudwammer.service.plugin.settings.Option;
import org.ncmud.mudwammer.service.plugin.settings.PluginSettings;
...
public class Plugin implements SettingsChangedListener {
    ...
    public Plugin(Handler h, ConnectionPluginCallback parent, String path, String dataDir)
            throws LuaException {
    ...
    */
    ...
    private class SendToServerFunction extends JavaFunction {
        ...
        public int execute() throws LuaException {
            ...
            // Log.e("LUAWINDOW","script is sending:"+this.getParam(2).getString()+" to server.");
            Plugin.this.mHandler.sendMessage(
                    Plugin.this.mHandler.obtainMessage(
                            Connection.MESSAGE_SENDDATA_STRING, this.getParam(2).getString()));
            return 0;
        }
    }
    ...
    \endluacode
    \param line \b window.TextTree$Line the line to append, this usually comes from a trigger callback
    \returns none
    \par Example
    \luacode
    ...
    */
    private class AppendLineToWindowFunction extends JavaFunction {
        ...
        @Override
        public int execute() throws LuaException {
            // the only arguments should be a TextTree.Line copied from the one passed to it from a
            ...
            if (o.isJavaObject()) {
                ...
                if (tmp instanceof TextTree.Line) {
                    ...
                    // now abuse the lineToWindow handler from the replace action to ferry this bad
                    // boy across
    ...
    private class AppendWindowSettingsFunction extends JavaFunction {
        ...
        public int execute() throws LuaException {
            ...
            if (w != null) {
                // mWindows.get(0).getSettings().setListener(new
                // WindowSettingsChangedListener(mWindows.get(0).getName()));
    ...
    */
    ...
    Gets the raw /c service.WindowToken object that is being held by the background service. This is to allow direct manipulation of the buffer.
    ...
    private class InvalidateWindowTextFunction extends JavaFunction {
        ...
        public int execute() throws LuaException {
            ...
            mHandler.sendMessage(
                    mHandler.obtainMessage(Connection.MESSAGE_INVALIDATEWINDOWTEXT, name));
            return 0;
        }
    }
    ...
    Gets the raw \b service.settings.SettingsGroup settings, this is to allow direct manipulation.
    ...
    \endluacode
    \param none
    \returns a \b service.settings.SettingsGroup object that can be directly manipulated.
    \par Example
    \luacode
    ...
    private class NewTriggerFunction extends JavaFunction {
        ...
        public int execute() throws LuaException {
            ...
            ArrayList<HashMap<String, Object>> responders =
            ...
            if (pScriptName.isNil()) {
                ...
                tok =
                        new WindowToken(
                                name,
                                scriptName,
                                Plugin.this.getSettings().getName(),
    ...
    */
    private class ReloadSettingsFunction extends JavaFunction {
        ...
        public int execute() throws LuaException {
            ...
            // Log.e("LUAWINDOW","script is sending:"+this.getParam(2).getString()+" to server.");
            Plugin.this.mHandler.sendMessage(
                    Plugin.this.mHandler.obtainMessage(Connection.MESSAGE_RELOADSETTINGS));
            return 0;
        }
    }
        ...
        public int execute() throws LuaException {
            ...
            Log.e(
                    ...
                    "SENDING FUNCTION:"
                            ...
                            + "("
                            + funcstring
                            + "): for inclusion into the global .command processor.");
    ...
    */
    private class SaveSettingsFunction extends JavaFunction {
        ...
        public int execute() throws LuaException {
            ...
            mHandler.sendMessage(
                    mHandler.obtainMessage(Connection.MESSAGE_SAVESETTINGS, Plugin.this.getName()));
            return 0;
        }
    }
    ...
    */
    private class SimulateInputFunction extends JavaFunction {
        ...
        public int execute() throws LuaException, UnsupportedEncodingException {
            ...
            mHandler.sendMessage(
                    mHandler.obtainMessage(Connection.MESSAGE_PROCESS, str.getBytes(mEncoding)));
    ...
    */
    ...
    public class TriggerEnabledFunction extends JavaFunction {
        ...
        public int execute() throws LuaException {
            ...
            if (Plugin.this.getSettings().getTriggers().containsKey(trigger)) {
                ...
                // return error
                L.pushString(
                        "Function: TriggerEnabled(string,boolean) Error: \""
                                + trigger
                                + "\" does not exist");
                return 1;
            }
    ...
    private class WindowBufferFunction extends JavaFunction {
        ...
        public int execute() throws LuaException {
            ...
            mHandler.sendMessage(
                    mHandler.obtainMessage(Connection.MESSAGE_WINDOWBUFFER, set ? 1 : 0, 0, win));
    ...
    public void execute(String callback, String args) {
        ...
        if (L.getLuaObject(-1).isFunction()) {
            ...
            if (ret != 0) {
                displayLuaError(
                        "Error calling function callback:"
                                + settings.getName()
                                + "("
                                + callback
                                + "):"
                ...
                // Log.e("PLUGIN","Successfuly called plugin
                // function:"+settings.getName()+"("+callback+")");
                L.pop(2);
            }
        ...
        if (a.length > 0) {
            ...
            for (int i = 0; i < a.length; i++) {
                if (((AliasData) a[i]).isEnabled()) {
                    if (((AliasData) a[i]).getPre().startsWith("^")) {
                        prefix = "";
                    } else {
                        prefix = "\\b";
                    }
                    if (((AliasData) a[i]).getPre().endsWith("$")) {
                        suffix = "";
                    } else {
                        suffix = "\\b";
                    }
    ...
    public byte[] doAliasReplacement(byte[] input, Boolean reprocess) {
        if (joined_alias.length() > 0) {
            ...
            try {
                alias_replacer.reset(
                        new String(input, mEncoding)); // replacer = to_replace.matcher(new
                // String(bytes,the_settings.getEncoding()));
            ...
            while (alias_replacer.find()) {
                ...
                if (startAnchor && !endAnchor) {
                    ...
                    if (finalString.startsWith(scriptBlock)) {
                        // if it didn't compile then we send as normal, if it did it gets eaten, but
                        // in this case eaten means "return the same caller string
                        // so that the visual string sent back to the
                        this.runLuaString(
                                finalString.substring(scriptBlock.length(), finalString.length()));
                    } else {
            ...
            if (found) { // if we replaced a match, we need to continue the find/match process until
                // none are found.
                ...
                do {
                    ...
                    while (alias_recursive.find()) {
                        ...
                        // AliasData replace_with =
                        // getSettings().getAliases().get(alias_recursive.group(0));
                        if (replace_with.getPre().startsWith("^")
                                && !replace_with.getPre().endsWith("$")) {
                            ToastResponder r = new ToastResponder();
                            String[] tParts = null;
                            ...
                            eatTail = true;
                            alias_recursive.appendReplacement(
                                    buffertemp,
                                    r.translate(replace_with.getPost(), map) + sepchar + rest);
                            reprocess = false;
                        } else {
        ...
        for (TriggerResponder responder : data.getResponders()) {
            try {
                responder.doResponse(
                        ...
                        0,
                        "",
                        (Object) getSettings().getTimers().get(ordinal),
                        parent.getDisplayName(),
                        parent.getHostName(),
                        parent.getPort(),
                        StellarService.getNotificationId(),
                        parent.isWindowShowing(),
                        mHandler,
                        captureMap,
                        L,
                        Plugin.this.getSettings().getTimers().get(ordinal).getName(),
                        mEncoding);
            } catch (IteratorModifiedException e) {
                // won't ever get here because gag/replace actions can't be applied to timers.
            }
            // service.
            // responder.doResponse(parent.getContext(),null,null,null,null,
            // parent.getDisplayName(), StellarService.getNotificationId(),
            // parent.isWindowShowing(), mHandler, null,L,data.getName());
        }
        ...
        if (d.isPlaying()) {
            ...
            if (d.getRemainingTime() != d.getSeconds()) {
                ...
                if (d.isRepeat()) {
                    ...
                    CONNECTION_TIMER.schedule(
                            task, d.getRemainingTime() * 1000, d.getSeconds() * 1000);
                } else {
                    d.setStartTime(startTime);
    ...
    public void buildTriggerSystem() {
        ...
        // Log.e("TRIGGERS","TIMEPROFILE "+getSettings().getName()+" trigger system took " + delta +
        // " millis to build.");
    }
        ...
        for (Option o : options) {
            if (o instanceof SettingsGroup) {
                ...
                if (L.getLuaObject(-1).isFunction()) {
                    ...
                    if (ret != 0) {
                        displayLuaError(
                                "Error in OnOptionChanged:" + L.getLuaObject(-1).getString());
                    } else {
                        L.pop(2);
                    }
                } else {
