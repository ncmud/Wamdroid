...
/**
 * Manages alias parsing, semicolon splitting, special command handling, and keyboard alias
 * replacement.
 */
public class AliasManager {
    ...
    private HashMap<String, SpecialCommand> mSpecialCommands =
            new HashMap<String, SpecialCommand>();
     ...
     * @throws UnsupportedEncodingException Problem with the String<==>byte[] conversion indicating
     *     a bad encoding option.
     */
    public Data processOutputData(final String data, final Connection connection)
        ...
        while (iterator.hasNext()) {
            ...
            if (cmd.endsWith("~")) {
                ...
                if (cmd.startsWith(mScriptBlock)) {
                    mContext.getConnectionSettings()
                            .runLuaString(cmd.substring(mScriptBlock.length(), cmd.length()));
                } else {
                ...
                if (d != null) {
                    ...
                    if (d.mCmdString != null && d.mVisString != null) {
                        if (d.mCmdString.equals(d.mVisString)) {
                            m = true; // aliases & regular commands will always have the same
                            // cmdString and visString
                        }
                    }
                    ...
                    if (d.mCmdString != null && !d.mCmdString.equals("")) {
                        ...
                        for (int i = 0; i < plugins.size() + 1; i++) {
                            ...
                            if (p.getSettings().getAliases().size() > 0) {
                                ...
                                tmp =
                                        p.doAliasReplacement(
                                                d.mCmdString.getBytes(
                                                        mContext.getConnectionSettings()
                                                                .getEncoding()),
                                                reprocess);
                                String tmpstr =
                                        new String(
                                                tmp,
                                                mContext.getConnectionSettings().getEncoding());
     ...
     * @param connection The connection instance needed by special commands.
     * @return The Data object containing the string to return to the server and the string to
    ...
    public final Data processCommand(final String cmd, final Connection connection) {
        ...
        if (cmd.equals(".." + "\n") || cmd.equals("..")) {
            synchronized (mContext.getConnectionSettings()) {
                ...
                try {
                    mContext.sendBytesToWindow(
                            outputmsg.getBytes(mContext.getConnectionSettings().getEncoding()));
        ...
        if (cmd.startsWith(".") && mContext.getConnectionSettings().isProcessPeriod()) {
            ...
            if (mCommandMatcher.find()) {
                synchronized (mContext.getConnectionSettings()) {
                    ...
                    if (mContext.getConnectionSettings()
                            .getSettings()
                            .getAliases()
                            .containsKey(alias)) {
                        // real argument
                        if (!argument.equals("")) {
                            ...
                            String noargMessage =
                                    "\n"
                                            ...
                                            + " Alias \""
                                            + alias
                                            + "\" can not be set to nothing. Acceptable format is"
                                            + " \"."
                                            + alias
                                            + " replacetext\""
                        ...
                        try {
                            mContext.sendBytesToWindow(
                                    error.getBytes(mContext.getConnectionSettings().getEncoding()));
                        } catch (UnsupportedEncodingException e) {
                            throw new RuntimeException(e);
                        }
                        return null;
