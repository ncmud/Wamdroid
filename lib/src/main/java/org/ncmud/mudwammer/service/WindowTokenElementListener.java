...
/**
 * Utility class that is used to inflate window token data from the xml settings through the SAX
 * parser.
 */
public class WindowTokenElementListener implements ElementListener {
     ...
     */
    public WindowTokenElementListener(
            final NewItemCallback callback, final WindowToken currentWindow) {
    ...
    public final void start(final Attributes a) {
        mCurrentWindow.setName(
                (a.getValue("", "name") == null) ? "mainDisplay" : a.getValue("", "name"));
        mCurrentWindow.setBufferText(
                (a.getValue("", "bufferText") == null)
                        ? false
                        : (a.getValue("", "bufferText").equals("true")) ? true : false);
        mCurrentWindow.setId(
                (a.getValue("", "id") == null) ? 0 : Integer.parseInt(a.getValue("", "id")));
        mCurrentWindow.setScriptName(
                (a.getValue("", "script") == null) ? "" : a.getValue("", "script"));
        // current_window.
    }
