...
public class TimerElementListener implements ElementListener {
    ...
    public void start(Attributes a) {
        current_timer.setName(
                (a.getValue("", BasePluginParser.ATTR_TIMERNAME) == null)
                        ? ""
                        : a.getValue("", BasePluginParser.ATTR_TIMERNAME));
        current_timer.setOrdinal(
                (a.getValue("", BasePluginParser.ATTR_ORDINAL) == null)
                        ? 0
                        : Integer.parseInt(a.getValue("", BasePluginParser.ATTR_ORDINAL)));
        ...
        current_timer.setSeconds(
                (a.getValue("", BasePluginParser.ATTR_SECONDS) == null)
                        ? 30
                        : Integer.parseInt(a.getValue("", BasePluginParser.ATTR_SECONDS)));
        ...
        current_timer.setRepeat(
                (a.getValue("", BasePluginParser.ATTR_REPEAT) == null)
                        ? false
                        : a.getValue("", BasePluginParser.ATTR_REPEAT).equals("true")
                                ? true
                                : false);
        current_timer.setPlaying(
                (a.getValue("", BasePluginParser.ATTR_PLAYING) == null)
                        ? false
                        : a.getValue("", BasePluginParser.ATTR_PLAYING).equals("true")
                                ? true
                                : false);
